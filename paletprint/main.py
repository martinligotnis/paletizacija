...
def printit():
  ...
  sql = "select * from venden1.paletes ORDER BY DatumsLaiks DESC LIMIT 10"
  cursor = conn.cursor()
  cursor.execute(sql)
  ...
  if InitCheck:
    for i in range(0, len(InitCheck)):
      ...
      if InitCheck[i][6] == 0:
        ...
        # Calculate the number of already existing paletes records for the current product on the given date.
        cursor = conn.cursor()
        cursor.execute(
